...
文件列表是**递归**的（`name` 是相对路径，如 `lib/util.py`），删除按钮也支持嵌套路径
—— 否则上传了文件夹，子目录里的文件在页面上既看不见也删不掉。

列表上方有一条工具栏，可以**多选**（表头全选，支持半选态）后：

| 操作 | 接口 | 说明 |
| --- | --- | --- |
| 删除选中 | `POST /api/toolgroups/{id}/files/delete` | body `{"names": ["lib/util.py", ...]}` |
| 清空全部 | `POST /api/toolgroups/{id}/files/clear` | 删掉除 `__init__.py` 外的全部文件 |

两条规则：

* **`__init__.py` 永远删不掉** —— 单删、批量、清空都会跳过它。它是这个包能被
  `import` 的前提，删了整个工具组就废了。所以「清空全部」的语义是
  「清空所有上传的文件，保留包骨架」。
* 删除后会**回收因此变空的目录**（从深到浅），不会留下一堆空壳。

批量删除走 POST + JSON body 而不是把路径拼进 URL，避免嵌套路径的编码问题；
穿越路径（`../`、绝对路径）在服务端逐条过 `_safe_relpath`，非法的直接跳过。
